...
##  {#webappslide-id data-menu-title="An Example Web App"}
...
##  {#webappslide-id data-menu-title="An Example Web App - Multiple Runs"}
...
with col2:
    if run_simulation:
        ...
        # Histogram of final waiting list lengths
        ...
        # Box plot showing variability over time (every 13 weeks)
        ...
        # Show detailed results from the first run for backward compatibility
        st.subheader("Detailed Analysis (First Run)")

        # Use the first run's detailed data
        first_run = run_single_simulation(42)
        patients_seen_data = first_run['patients_seen_data']
        all_patients_data = first_run['all_patients']
        ...
        if patients_seen_data:
            ...
            st.write("**Patients Who Were Seen (First Run):**")
            col3a, col3b, col3c = st.columns(3)
            ...
            with col3a:
                st.metric(
                    label="Waited > 18 weeks",
                    value=over_18_weeks,
                    delta=f"{(over_18_weeks/len(patients_seen_data)*100):.1f}%"
                )
            ...
            with col3b:
                st.metric(
                    label="Waited > 36 weeks",
                    value=over_36_weeks,
                    delta=f"{(over_36_weeks/len(patients_seen_data)*100):.1f}%"
                )
            ...
            with col3c:
                st.metric(
                    label="Waited > 52 weeks",
                    value=over_52_weeks,
                    delta=f"{(over_52_weeks/len(patients_seen_data)*100):.1f}%"
                )
