...
class Scenario():
    ...
    def __init__(self, run_length, warm_up=0.0, pooling=False, prop_carve_out=0.15,
        ...
        if pooling_file is None:
            pooling_file = pd.read_csv('animation_examples/complex_community_booking/data/partial_pooling.csv')

        if demand_file is None:
            demand_file = pd.read_csv('animation_examples/complex_community_booking/data/referrals.csv')

        if slots_file is None:
            slots_file = pd.read_csv('animation_examples/complex_community_booking/data/shifts.csv')
...
class PatientReferral(object):
    ...
    '''
    def __init__(self, env, args, referral_t, home_clinic, booker, event_log, identifier):
        self.env = env
        self.args = args
            ...
            for i in range(n_referrals):
                ...
                if referred_out == 0:
                    ...
                    patient = PatientReferral(self.env, self.args, t,
                                              clinic_id, booker, self.event_log, f"{t}_{i}")
